...
export const TimelineStepResizable = memo(
        ...
        >
          <Box
            ...
            sx={{
              ...
              boxShadow:
                isDragging || isResizing ? '0 4px 20px rgba(0,0,0,0.2)' : isHovered ? 2 : 1,
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Typography
                  ...
                  sx={{
                    ...
                    flex: displayPosition.width > 150 ? 'none' : 1,
                  }}
                >
                  {step.title}
                </Typography>
                {/* Department chips inline with title - only show if width > 100px */}
                {displayPosition.width > 100 &&
                  step.associatedDepartments &&
                  step.associatedDepartments.length > 0 && (
                    <Box sx={{ display: 'flex', gap: 0.3, flexShrink: 0 }}>
                      {step.associatedDepartments.map((dept) => (
                        <Chip
                          key={dept}
                          label={getDepartmentAcronym(dept)}
                          size="small"
                          sx={{
                            backgroundColor: getDepartmentColor(dept),
                            color: '#FFFFFF',
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.step.associatedDepartments) ===
        JSON.stringify(nextProps.step.associatedDepartments) &&
